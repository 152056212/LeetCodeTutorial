...
public class JunitLeetcode01 {
	 ...
	 * 示例 1:
	 * <p>
	 * 输入: dividend = 10, divisor = 3 输出: 3 解释: 10/3 = truncate(3.33333..) =
	 * truncate(3) = 3
	 * <p>
	 * 示例 2:
	 * <p>
	 ...
	 * 提示：
	 * <p>
	 * 被除数和除数均为 32 位有符号整数。 除数不为 0。 假设我们的环境只能存储 32 位有符号整数，其数值范围是 [−2E31, 2E31 −
	 * 1]。本题中，如果除法结果溢出，则返回 2E31 − 1。
	 * <p>
	...
	public void testDivisor() {
		...
		if (dividend != Integer.MIN_VALUE) {
			modNum = Math.abs(dividend);
		} else {
			modNum = Math.abs(dividend + Math.abs(divisor));
			res++;
		}
		while (tmp <= modNum) {
			modNum = modNum - tmp;
	...
	public void testTreeMap() {
		...
		TreeMap<Integer,ArrayList<Integer>> treeMap = new TreeMap<Integer,ArrayList<Integer>>(Comparator.reverseOrder());
		treeMap.comparator();
		...
		treeMap.put(3, lists);
		int i = 0;
		Iterator<Integer> keysItor =treeMap.keySet().iterator();
		while(i < 10 && keysItor.hasNext()) {
			i++;
			System.out.println(keysItor.next());
		}
		treeMap.entrySet();
	...
	public void testBalancedTree() {
		...
		List<List<String>> res = new ArrayList<List<String>>(1);
        ...
        for(int i = 0; i < strs.length; i++) {
        	...
        	System.out.println(strs[i]+" tmp==> "+tmp);
        	List<String> listInMap = null;
        	if(map.get(tmp) == null) {
        		listInMap = new ArrayList<>();
        		listInMap.add(strs[i]);
        		map.put(tmp, listInMap);
        	}else {
        		listInMap = map.get(tmp);
        		listInMap.add(strs[i]);
        	}
        }
